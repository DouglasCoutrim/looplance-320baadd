    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-secondary mt-1 font-medium text-base sm:text-lg">
            Monitoramento e controle da infraestrutura <span className="text-brand font-bold">Edge Replay</span>.
          </p>
        </div>
        <div className="hidden sm:flex items-center gap-3">
          <div className="h-12 w-12 rounded-full bg-brand brand-glow flex items-center justify-center text-white">
            <ShieldCheck className="h-6 w-6" />
          </div>
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-muted">Status do Sistema</p>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 glass-card bg-surface p-4 sm:p-6 shadow-subtle border border-border">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-lg sm:text-xl font-black text-primary flex items-center gap-2 uppercase tracking-tight">
              <Activity className="h-5 w-5 text-brand" />
              Distribuição de Infra
            </h3>
            <div className="inline-flex items-center gap-1 text-xs font-bold text-brand-text bg-brand-dim px-2 py-1 rounded-md">
        ...
        <div className="lg:col-span-2 space-y-4">
          <h3 className="text-lg sm:text-xl font-black text-primary flex items-center gap-2 uppercase tracking-tight px-1">
          ...
          <div className="glass-card bg-brand brand-glow p-4 sm:p-6 text-white mt-2 relative overflow-hidden group transition-transform hover:scale-[1.02]">
            <div className="relative z-10">
              ...
              <button className="mt-4 inline-flex items-center gap-2 rounded-full bg-white/10 backdrop-blur-md px-4 py-2 text-xs font-black uppercase tracking-widest hover:bg-white/20 transition">
...
function StatCard({ title, value, icon, description, link, color = "orange" }: any) {
  ...
  const content = (
    <div className="glass-card bg-surface p-4 sm:p-6 shadow-subtle border border-border transition-all hover:shadow-xl hover:-translate-y-1 group">
      <div className="flex items-center justify-between mb-4">
        <div className={`p-2.5 sm:p-3 rounded-2xl ${colorClasses[color] || colorClasses.orange} transition-colors group-hover:bg-brand group-hover:text-white`}>
          {icon}
        </div>
        <div className="text-2xl sm:text-3xl font-black text-primary">{value}</div>
      </div>
      <div>
        <h4 className="text-[10px] sm:text-xs font-black uppercase tracking-[0.1em] text-muted">{title}</h4>
        <p className="text-xs sm:text-sm font-medium text-secondary mt-1">{description}</p>
      </div>
    </div>
  );
...
function QuickAction({ to, title, description, icon }: any) {
  return (
    <Link 
      to={to} 
      className="flex items-center gap-3 sm:gap-4 glass-card bg-surface p-3 sm:p-4 shadow-sm border border-border transition-all hover:border-brand/50 hover:shadow-subtle hover:bg-bg-card-hover group"
    >
      <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-tag flex items-center justify-center text-muted group-hover:bg-brand group-hover:text-white transition-all">
        {icon}
      </div>
      <div className="flex-1">
        <div className="font-black uppercase tracking-tight text-xs sm:text-sm text-primary">{title}</div>
        <div className="text-[10px] sm:text-xs font-medium text-secondary mt-0.5">{description}</div>
      </div>
      <ArrowRight className="h-4 w-4 text-muted group-hover:text-brand group-hover:translate-x-1 transition-all" />
    </Link>
  );
}
